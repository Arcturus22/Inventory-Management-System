...
namespace IMS.Plugins.EFCoreSqlServer
{
    public class InventoryEFCoreRepository : IInventoryRepository
    {
        ...
        public async Task<IEnumerable<Inventory>> GetInventoriesByNameAsync(string name)
        {
            ...
            //return await db.Inventories?.Where(x => x.InventoryName.ToLower().Contains(name.ToLower())).ToListAsync();
        }   
        ...
        public async Task<Inventory> GetInventoryByIdAsync(int inventoryId)
        {
            using var db = contextFactory.CreateDbContext();
            var inventory = await db.Inventories.FindAsync(inventoryId);
            //var inventory = db.Inventories.Where(x => x.InventoryId == inventoryId);
